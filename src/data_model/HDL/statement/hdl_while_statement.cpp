...
CEREAL_REGISTER_TYPE(hdl_while_statement)
CEREAL_REGISTER_POLYMORPHIC_RELATION(hdl_statement_base, hdl_while_statement)
...
bool hdl_while_statement::equals(const hdl_statement_base &other) const {
    const auto& rhs = static_cast<const hdl_while_statement&>(other);
...
std::string hdl_while_statement::print() const {
    std::ostringstream oss;
    oss << "while( " << (end_condition ? end_condition->print() : "") << " ) begin\n";
    for (auto &stmt:loop_body) {
...
void PrintTo(const hdl_while_statement& s, std::ostream* os) {
    *os << s.print();
}
